...
def detect_mentions_df(df: pd.DataFrame) -> pd.DataFrame:
    mentions = df["text"].fillna("").apply(detect_icpsr_mentions)
    df = df.copy()
    df["icpsr_ids"] = mentions.apply(lambda m: m["icpsr_ids"])
    df["icpsr_dois"] = mentions.apply(lambda m: m["icpsr_dois"])
    df["icpsr_cues"] = mentions.apply(lambda m: m["icpsr_cues"])
    df["has_icpsr"] = df["icpsr_ids"].apply(bool) | df["icpsr_dois"].apply(bool)
    return df

def fetch_icpsr_meta_for_df(df: pd.DataFrame) -> pd.DataFrame:
    ids = set()
    for _, row in df.iterrows():
        for i in (row.get("icpsr_ids") or []):
            ids.add(i)
